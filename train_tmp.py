...
def scene_reconstruction(gaussians, scene, gaussians_renderer, stage, tb_writer):
    ...
    # for iteration in range(start_iter, training_args.iterations + 1):
        ...
        render_pkg = gaussians_renderer.render(viewpoint_cam, gaussians, return_dx=cfg.model.nsg.use_deformation_model)
...
def training():
    ...
    try:
        trained_model_dir = os.path.join("/home/ml4u/BKTeam/TheHiep/street_gaussians/output_street_gaussian_wo_3DOD/waymo_exp", cfg.exp_name, "trained_model")
        if cfg.loaded_iter == -1:
            loaded_iter = searchForMaxIteration(trained_model_dir)
        else:
            loaded_iter = cfg.loaded_iter
        if cfg.model.nsg.use_deformation_model: 
            gaussians.load_state_dict(trained_model_dir, loaded_iter)
        else:
