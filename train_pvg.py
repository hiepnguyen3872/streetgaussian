...
from lib.utils.loss_utils import l1_loss, l2_loss, psnr, ssim, silog_loss, get_dynamic_mask
from lib.utils.img_utils import save_img_torch, visualize_depth_numpy, visualize_segmentation
...
def scene_reconstruction(gaussians, scene, gaussians_renderer, stage, tb_writer):
    ...
    for iteration in range(start_iter, training_args.iterations + 1):
        ...
        v = gaussians.background.get_inst_velocity
        t_scale = gaussians.background.get_scaling_t.clamp_max(2)
        other = [t_scale, v]
        
        # time_duration = [-args.frame_interval*(frame_num-1)/2,args.frame_interval*(frame_num-1)/2] 
        frame_num = len(viewpoint_stack)       
        time_duration = [-0.02*(frame_num-1)/2,0.02*(frame_num-1)/2]  
        ...
        if cfg.model.mask_2d_dynamic_object and cfg.data.use_semantic: 
        ...
        render_pkg = gaussians_renderer.render(viewpoint_cam, gaussians, return_dx=cfg.model.nsg.use_deformation_model, use_hexplane=use_hexplane,other=other ,time_shift=time_shift)
        ...
        if optim_args.lambda_depth_lidar > 0 and 'pseudo_depth' in viewpoint_cam.meta:            
            lidar_depth = viewpoint_cam.meta['pseudo_depth'].cuda() # [1, H, W]
            lidar_depth_loss = silog_loss(depth[mask], lidar_depth[mask])
            loss += optim_args.lambda_depth_lidar * lidar_depth_loss
        ...
        with torch.no_grad():
            ...
            if iteration < optim_args.densify_until_iter:
                ...
                if iteration > optim_args.densify_from_iter:
                    if iteration % optim_args.densification_interval == 0:
                        gaussians.background.densify_and_prune(
            ...
            if (iteration in training_args.checkpoint_iterations):
                ...
                ckpt_path = os.path.join(cfg.trained_model_dir, f'iteration_{iteration}_sky.pth')
                ...
                torch.save((gaussians.background.capture(), iteration), ckpt_path)
