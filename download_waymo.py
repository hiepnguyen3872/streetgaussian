...
def download_files(
    file_names: List[str],
    target_dir: str,
    source: str = "gs://waymo_open_dataset_scene_flow/valid",
) -> None:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target_dir",
        type=str,
        default="/media/ml4u/ExtremeSSD/datasets/waymo/raw",
    ...
    total_list = open("script/waymo/waymo_splits/segment_list_val_short.txt", "r").readlines()
    if args.split_file is None:
        # file_names = [total_list[i].strip() for i in args.scene_ids]
        file_names = [total_list[i].strip() for i in range(len(total_list))]
    else:
...
# python script/waymo/generate_lidar_depth.py --datadir '/media/ml4u/ExtremeSSD/datasets/waymo/processed/181'
